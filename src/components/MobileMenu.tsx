    ...
    <div className="border-b border-slate-100 last:border-0">
      {isHeading ? (
        ...
        <Link 
          to={group.to}
          onClick={closeMenu}
          className="block w-full text-[18px] font-bold text-[#0d315c] py-6 hover:text-[#019e6e] transition-colors"
        >
          {group.title}
        </Link>
      )}
    ...
    >
      ...
      >
        ...
        <div className="flex items-center justify-end px-6 py-5 shrink-0">
          <button 
            onClick={closeMenu}
            className="w-10 h-10 rounded-full bg-slate-50 flex items-center justify-center text-[#0d315c] hover:bg-slate-100 active:scale-90 transition-all duration-200"
