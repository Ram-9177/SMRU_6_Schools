...
const viewportFitFallbackHeight = (iframe: HTMLIFrameElement | null) => {
  ...
  const minViewportFill = Math.round(viewportHeight * 0.72);
  const fallbackFloor = Math.max(760, minViewportFill);
  ...
  return clampHeight(Math.max(fallbackFloor, viewportHeight - topOffset - footerHeight - safeGap));
};
