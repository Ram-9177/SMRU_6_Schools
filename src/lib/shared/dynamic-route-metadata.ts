...
export const getSchoolMetadata = (params: { schoolSlug: string }): Metadata => {
  ...
  const title = `${schoolName} Admissions 2026 | SMRU`;
  ...
  return buildMetadata({
    ...
    keywords: [schoolName, "Admissions 2026", "Hyderabad University", "College Telangana", "SMRU", ...customKeywords],
  });
};
...
export const getDepartmentMetadata = (params: { schoolSlug: string; deptSlug: string }): Metadata => {
  ...
  const description = dept?.about 
    ? `${dept.about.substring(0, 160)}... Join the leading department for specialized studies at St. Mary's Rehabilitation University.`
...
export const getProgramMetadata = (params: { schoolSlug: string; deptSlug: string; programSlug: string }): Metadata => {
  ...
  // Authority Pattern: [Program Name] Admissions 2026 | SMRU
