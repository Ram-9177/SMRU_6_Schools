...
const HIDDEN_PARTNER_CODES = new Set(["KPMG", "MICROSOFT"]);
...
export function loadSeedState(): DeveloperCMSState {
  ...
  Object.values(EDU_PARTNERS || {}).forEach((partner: any) => {
    ...
    partners.push({
      id,
      slug: (partner.landingUrl || "").replace(/^\//, "") || (partner.code || "").toLowerCase(),
      name: partner.name,
    ...
    (school.departments || []).forEach((department: any, deptIndex: number) => {
      ...
      (department.programs || []).forEach((program: any, programIndex: number) => {
        ...
        programs.push({
          ...
          higherStudies: "",
          placementNote: program.salaryNote || "Top placement record",
          ...
          trainingPartnerAssigned: program.partnerCode ? [program.partnerCode] : [],
          indexable: true,
          inSitemap: true,
          visibility: defaultVisibility,
          status: defaultStatus,
        ...
        if (program.partnerCode) {
          const partnerId = partnerCodeToId.get(String(program.partnerCode).toUpperCase());
          if (partnerId) {
            partnerLinks.push({
              ...
              redirectLink: program.partnerLeadUrl || normalizePath(EDU_PARTNERS[program.partnerCode]?.landingUrl) || "",
        ...
        if (programIndex === 0 && !program.partnerCode) {
          partnerLinks.push({
