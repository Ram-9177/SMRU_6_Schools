    ...
    schoolSource.forEach((school) => {
      school.departments?.forEach((dept) => {
        dept.programs?.forEach((prog) => {
          ...
          const uniqueKey = `${school.name}-${category}-${canon}`;
          ...
          if (!seen.has(uniqueKey)) {
            ...
            list.push({
              ...
              progName: cleanProgramName(prog.name, { trailingOnly: true }),
  ...
  return (
    <div className="min-h-screen bg-white font-outfit text-[#0d315c] selection:bg-[#019e6e]/10">
      <SEO 
        ...
        description="Explore the complete academic catalogue of St. Mary's University. Over 70 programs across diverse schools of engineering, rehabilitation, and health sciences."
      />
        ...
        <div className="relative z-10 container mx-auto px-6 text-center space-y-10 max-w-5xl">
          ...
          >
            ...
            <p className="mt-8 text-lg md:text-2xl text-slate-500 font-medium italic leading-relaxed opacity-80 max-w-3xl mx-auto">
              A comprehensive architecture of clinical mastery, engineering innovation, and research leadership across St. Mary's University's specialized schools.
            </p>
          </motion.div>
      ...
      <section className="relative z-20 -mt-16 container mx-auto px-6 max-w-7xl">
        <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
          {[
            ...
            { label: "Doctoral / PhD", count: stats.phd, icon: <FaBrain />, color: "from-purple-600 to-purple-800" },
            ...
            >
              {s.label === "Doctoral / PhD" ? (
                <Link to="/phd-admissions" className="flex flex-col items-center gap-4 group">
        ...
        <div className="bg-[#fff9ef] border-l-8 border-[#ffaf3a] p-8 cut-corner-panel shadow-sm flex items-center gap-6">
          ...
          <div className="space-y-1">
            ...
            <p className="text-slate-600 text-sm font-medium leading-relaxed">
              Our website is currently being updated for the upcoming academic session. All program structures listed below are valid for the 2026-27 cycle. Applications are now open for select programs.
            </p>
          </div>
        </div>
      </section>
        ...
        <div className="container mx-auto px-6 max-w-7xl">
          <div className="flex flex-col md:flex-row gap-8 items-end justify-between mb-12">
            <div className="space-y-4 max-w-2xl">
              ...
              <p className="text-slate-400 text-sm font-medium italic">Search through our diverse offerings to find your ideal path.</p>
          ...
          <div className="bg-white cut-corner-panel shadow-2xl border border-slate-100 overflow-hidden group">
                      ...
                      filtered.map((item, idx) => (
                        ...
                        >
                          <td className="px-10 py-8">
                            <div className="space-y-1">
                              <Link to={`/schools/${item.schoolSlug}`} className="text-[10px] font-black text-[#019e6e] uppercase tracking-widest hover:underline block">
                                {item.schoolName}
                              </Link>
                              <Link to={`/schools/${item.schoolSlug}/${item.deptSlug}`} className="text-lg font-black tracking-tight text-[#0d315c] hover:text-[#019e6e] transition-colors block">
                                {item.deptName}
                              </Link>
                            </div>
                          </td>
                          ...
                          <td className="px-10 py-8">
                            <Link 
                              to={`/schools/${item.schoolSlug}/${item.deptSlug}/${item.progSlug}`}
            ...
            <div className="bg-[#fcfdfd] px-10 py-8 border-t border-slate-100 flex flex-col md:flex-row justify-between items-center gap-4">
              ...
              <div className="flex gap-4 text-[11px] font-black text-[#019e6e] uppercase tracking-widest">
                ...
                <span className="flex items-center gap-2"><FaGlobeAmericas /> Global Recognition</span>
              </div>
            </div>
          </div>
        </div>
      ...
      {/* -------------------------------------------------------------------
          FINALE SECTION
          ------------------------------------------------------------------- */}
      <section className="py-32 bg-[#0d315c] text-white relative overflow-hidden">
        ...
        <div className="container mx-auto px-6 text-center space-y-12 relative z-10">
          ...
          <div className="space-y-4">
            ...
            <p className="text-white/60 text-lg md:text-xl font-medium max-w-2xl mx-auto italic px-6">
              "Every great journey begins with a single question. Our counselors are here to help you architect your future."
            </p>
          </div>
          <div className="pt-8 flex flex-wrap justify-center gap-6">
            <Link to="/contact" className="px-10 py-5 bg-[#019e6e] text-white font-black text-sm uppercase tracking-[0.2em] cut-corner-badge hover:scale-105 transition-all shadow-2xl">
              Talk to a Counselor
            </Link>
